...
class CountryData
  ...
  def self.population_less_than_20_million
    result = DatabaseConnection.query("SELECT * FROM countries WHERE population < 200;")
    result.map { |country| CountryData.new(country["name"], country["continent"], country["population"], country["density"], country["gni"]) }
  end
  ...
  def self.third_lowest_gni_per_capita_in_south_america
    result = DatabaseConnection.query("SELECT * FROM countries WHERE continent = 'South America' ORDER BY gni DESC LIMIT 1;")
    result.map { |country| CountryData.new(country["name"], country["continent"], country["population"], country["density"], country["gni"]) }
  end
  ...
  def self.countries_not_in_asia_hide_gni_per_capita_and_population_density
    result = DatabaseConnection.query("SELECT name, continent, population FROM countries WHERE continent NOT IN ('Asia');")
    result.map { |country| CountryData.new(country["name"], country["continent"], country["population"], country["density"], country["gni"]) }
  end
end
